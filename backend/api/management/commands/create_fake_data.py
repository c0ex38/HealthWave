...
class Command(BaseCommand):
    help = 'Generate fake users and data for testing'
    ...
    def handle(self, *args, **options):
        if Faker is None:
            self.stdout.write(
                self.style.ERROR('Faker library not installed. Install with: pip install faker')
            )
            return
        ...
        # Create fake users
        users = self.create_fake_users(fake, num_users)
        
        # Create data for each user
        for user in users:
            self.create_user_data(fake, user, num_days)
    ...
    def create_fake_users(self, fake, num_users):
        """Create fake users"""
        users = []
    ...
    def create_user_data(self, fake, user, num_days):
        ...
        for day_offset in range(num_days):
            ...
            if random.random() < 0.8:
                daily_log, created = self.create_daily_log(fake, user, current_date, current_datetime)
                if created:
                ...
                if random.random() < 0.4:
                    ...
                    self.create_chatbot_log(fake, user, chat_datetime)
    ...
    def create_daily_log(self, fake, user, date, created_at):
    ...
    def create_chatbot_log(self, fake, user, created_at):
        ...
        message_types = ['question', 'request', 'general', 'feedback']
